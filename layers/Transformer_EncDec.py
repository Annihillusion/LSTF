...
class gMLPDecoder(nn.Module):
    def __init__(self, layers, norm_layer=None, split = False):
        super(gMLPDecoder, self).__init__()
        self.layers = nn.ModuleList(layers)
        self.norm = norm_layer
        self.split = split
...
class gMLPDecoderLayer(nn.Module):
    def __init__(self, gMLPBlock, cross_attention, d_model, d_ff=None,
                 dropout=0.1, activation="relu"):
        super(gMLPDecoderLayer, self).__init__()
        d_ff = d_ff or 4*d_model
        self.gMLPblock = gMLPBlock
        self.cross_attention = cross_attention
        self.conv1 = nn.Conv1d(in_channels=d_model, out_channels=d_ff, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=d_ff, out_channels=d_model, kernel_size=1)
        self.norm1 = nn.LayerNorm(d_model)
